...
void b2GridShape::ComputeAABB(b2AABB* aabb, const b2Transform& transform, int32 childIndex) const
{
    ...
    float x0 = m_cellWidth * col - m_radius;
    float x1 = m_cellWidth * (col + 1) + m_radius;
    float y0 = m_cellHeight * row - m_radius;
    float y1 = m_cellHeight * (row + 1) + m_radius;
...
void b2GridShape::ComputeMass(b2MassData* massData, float density) const
{
    ...
    float w = m_cellHeight * m_rowCount;
    float h = m_cellWidth * m_columnCount;
    float area = w * h;
...
uint32 b2GridShape::GetCellVertices(uint32 index, b2Vec2* vertices) const
{
    ...
    float halfWidth = m_cellWidth * m_columnCount * 0.5f;
    float halfHeight = m_cellHeight * m_rowCount * 0.5f;
...
static bool hasEdge(b2Vec2 p0, b2Vec2 p1, b2Vec2* vertices, uint32 n, float cellWidth, float cellHeight)
{
    ...
    float absTol = epsilon * b2Max(cellWidth, cellHeight);
    ...
    for (uint32 i = 0; i < n; ++i)
    {
        uint32 i1 = i % n;
        uint32 i2 = (i+1) % n;
        float d1 = b2DistanceSquared(p0, vertices[i1]);
        float d2 = b2DistanceSquared(p1, vertices[i2]);
